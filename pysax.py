...
from itertools import starmap # added by SriniK
...
class SAXModel(object):
	def __init__(self, window = None, stride = None, 
		...
		vecs = list(starmap(lambda a, b: (a+b)/2, zip(cps, cps[1:]))) ## taking mean may not be accurate # edited by SriniK
		vecs[0] = cps[1]
		vecs[-1] = cps[-2]
	...
	def binpack(self, xs):
		...
		wts = [1 for _ in range(int(binsize))] + [binsize-int(binsize)] # edited by SriniK
		pos = 0
		while pos < len(xs):
			...
			wts = [1-wts[-1]] + [1 for _ in range(int(rest_wts))] + [rest_wts-int(rest_wts)] # edited by SriniK
	...
	def paa_window(self, window_signal):
		...
		xs = list(map(lambda ss: np.sum(ss) / float(binsize), self.binpack(s))) # edited by SriniK
		return xs
	...
	def symbolize_signal(self, signal, parallel = None, n_jobs = -1):
		...
		if parallel == None:
			return list(map(lambda wi: self.symbolize_window(signal[wi]), window_index)) # edited by SriniK
